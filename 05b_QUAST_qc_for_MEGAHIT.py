...
def run_quast(id_list, input_dir, output_dir, threads, min_contig_len):
    ...
    """
    Runs QUAST (Quality Assessment Tool for Genome Assemblies) on MEGAHIT output.
    ...
    # Loop through each assembly and run QUAST on the MEGAHIT contigs
    for id in id_list:
        print(f"Running QUAST for sample: {id}")
        
        # Define input and output paths for QUAST (assuming MEGAHIT contigs structure: 
        # {id}_megahit_output/final.contigs.fa)
        input_path = f"{input_dir}/{id}_megahit_output/final.contigs.fa"
        output_path = f"{output_dir}/{id}_megahit_quast"

        # Construct the QUAST command to run the analysis
        quast_cmd = (
...
if __name__ == "__main__":
    ...
    # Input directory should be the output directory from the MEGAHIT assembly step
